...
async fn main() -> Result<()> { //Объявление асинхронной функции main(), которая возвращает Result<()>. Это означает, что функция может выполняться асинхронно и может вернуть либо () в случае успеха, либо ошибку типа Result
    ...
    let mc = ModelController::new().await?; //Создание
    // нового экземпляра ModelController с помощью его асинхронного метода
    // new(), который возвращает Future. Оператор await ожидает завершения
    // этой операции. Затем ? обрабатывает ошибку, если она возникает при
    // инициализации ModelController
